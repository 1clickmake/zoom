...
if(isset($category) && is_array($category)){
    ...
    $in_list = "'".implode("','", $category)."'";
    $sql_search2 = " and ca_name IN({$in_list}) ";
}
...
if ($sca || $stx || $stx === '0') {     //검색이면
    ...
    $sql_search .= " and (wr_num between {$spt} and ({$spt} + {$config['cf_search_part']})) {$sql_search2} "; //////////////////////////////////////////////////
    ...
    */
    ...
    $sql_search = "";
    $sql = " select count(*) as cnt from {$write_table} where wr_is_comment = 0 {$sql_search2}"; //////////////////////////////////////////////////
    $row = sql_fetch($sql);
    $total_count = $row['cnt'];
...
if ($is_search_bbs) {
    ...
    $sql = " select * from {$write_table} where wr_is_comment = 0 {$sql_search2} ";/////////////////////////////////////////////
    if(!empty($notice_array))
